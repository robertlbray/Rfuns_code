make.table <- 
  function(
    ...
    #drop0trailing: Drops trailing zeros
    ...
    round.digits <- paste("%.", round.digits, "f", sep="")
    ...
    if(names.change){
      names <- read.delim(names, header=T, sep='\t', stringsAsFactors=F)
    ...
    #Headers
    relation <- ddply(d, col.vars, function(s) data.frame(column = 1))
    header <- relation
    if(ncols > 1){
      for(i in ncols:2){
        var <- col.vars[i]
    ...
    #Latex column names
    latex <- ldply(seq(ncols), function(i){
      var <- col.vars[i]
      x <- header
      x$.variable <- relation[, var]
      x <- ddply(x, c(var, '.variable'), function(s) data.frame(n = nrow(s)))
    ...
    if(table.type=='Bootstrap'){
      s <- ddply(d, names(d)[-which(names(d)%in%c('samp', 'value'))], function(s) data.frame(se=sd(s$value)))
    ...
    if(table.type != 'Summary'){
      d <- mutate(d, standard.err.space = ifelse(row.type == 'se', '\\\\[-6 pt]', ''))
    ...
    if(include.col.names){
      print(xtable(d, align=align.cols), include.rownames=F, include.colnames=F, floating=F, sanitize.text=identity, add.to.row=list(pos=pos, command=unlist(command)), hline.after=hline.value)
      print(xtable(d, align=align.cols), file=out.file, include.rownames=F, include.colnames=F, floating=F, sanitize.text=identity, add.to.row=list(pos=pos, command=unlist(command)), hline.after=hline.value)
    } else {
      print(xtable(d, align=align.cols), include.rownames=F, include.colnames=F, floating=F, sanitize.text=identity, hline.after=hline.value)
      print(xtable(d, align=align.cols), file=out.file, include.rownames=F, include.colnames=F, floating=F, sanitize.text=identity, hline.after=hline.value)
    }
    
    #Run sed files
    if(!is.na(sed.files)){
      l_ply(sed.files, function(i){
